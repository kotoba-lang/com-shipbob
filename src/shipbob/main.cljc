(ns shipbob.main
  "Kotodama WASM entrypoint for the Shipbob clean-room actor (L4) — Clojure port.
  ...
  facts (`shipbob.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "shipbob")
(def tier "L4")
...
(def entity-specs
  [{:entity "ContentEntry" :plural "contententries" :id-prefix "shipbob_con"
    :fields [:modelName :title :locale :published] :required [:modelName :title]
    :coerce {:published :bool}                :refs {}}
   {:entity "Product"      :plural "products"       :id-prefix "shipbob_pro"
    :fields [:sku :title :price :currency]         :required [:sku :title]
    :coerce {:price :float}                   :refs {}}
   {:entity "Shipment"     :plural "shipments"      :id-prefix "shipbob_shi"
    :fields [:orderId :carrier :tracking :status]  :required [:carrier :tracking]
    :coerce {}                                :refs {:orderId "Order"}}
   {:entity "SearchIndex"  :plural "searchindexes"  :id-prefix "shipbob_sea"
    :fields [:name :recordCount :primaryKey]       :required [:name :recordCount]
    :coerce {:recordCount :int}               :refs {}}
   {:entity "Webhook"      :plural "webhooks"       :id-prefix "shipbob_web"
    :fields [:event :url :active]                  :required [:event :url]
    :coerce {:active :bool}                   :refs {}}
   {:entity "Order"        :plural "orders"         :id-prefix "shipbob_ord"
    :fields [:number :total :currency :status]     :required [:number :total]
    :coerce {:total :float}                   :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"shipbob.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "shipbob-compat" :tier tier :entities entities} 200])
...
(defn start! [] :shipbob-compat/ready)
